...
// есть 1 баг. не считает победы, если выигрывает человек. а победы компьютера считает.
...
public class Start extends Applet implements ActionListener {
	...
	String lookForWinner() {
		...
		} else if (!squares[0].getLabel().equals("") && squares[0].getLabel().equals(squares[4].getLabel())
				&& squares[0].getLabel().equals(squares[8].getLabel())) {
			theWinner = squares[0].getLabel();
			highlightWinner(0, 4, 8);
			// Проверяем вторую диагональ – элементы массива 2,4,6
		} else if (!squares[2].getLabel().equals("") && squares[2].getLabel().equals(squares[4].getLabel())
