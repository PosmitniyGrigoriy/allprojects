...
// Если сравнивается int == double или int == byte или другие типы byte == short, или другое что-то через ==, то так писать можно и будет правильно выводиться тру или фолс.
// Иквелс нельзя использовать, так как это примитивные типы данных, а не объекты. Метода иквелс нет. Только ==.
// Если написать вместо int в Integer и так по другим типам данных, то == уже будет выдавать компиляционную ошибки, если сравниваются 2 разных wrapper класса. Если одни и те же классы сравниваются через ==, то там можно сделать. Integer == Integer.
// Нельзя сравнивать Integer == Double. Один тип сравнивать с этим же типом можно, но не с другим.
// Если сравнивать разные wrapper классы через иквелс, то компиляционной ошибки не будет, .но во время ран тайм баиндинг будет фолс, так как сравниваются 2 разных класса, а это 2 разных объекта, а раз это два разных объекта (Short и Double, например), то объекты не могут быть равны. Поэтому фолс. Если сравниваются два одинаковых wrapper класса через иквелс, но два разных значения, то будет фолс, если одинаковые значения, то будет тру.
